...
"Geçen ay en çok satan 5 ürün hangileri?"
        ↓
  [Önbellek: Son 24 saatte aynı sorgu var mı? → Varsa direkt döndür ⚡]
        ↓ (önbellekte yoksa)
...
| 1b | `database.py` | `get_cached_query()` ile TTL kontrolü; hit varsa LLM atlanır, SQL yeniden çalıştırılıp `from_cache:true` döner |
...
| ⚡ **Sorgu Önbellekleme** | Aynı sorgu 24 saat içinde tekrar gelirse LLM'e hiç gidilmez; `history` tablosundan SQL çekilip yeniden çalıştırılır. TTL ayarlanabilir, "Önbelleği atla" checkbox'ı ile devre dışı bırakılabilir |
...
| `use_cache` | bool | `true` | `false` gönderilirse önbellek atlanır, LLM doğrudan çağrılır |

**Response (başarılı SQL — LLM):**
```json
{
  "success": true,
...
**Response (başarılı SQL — önbellekten ⚡):**
```json
{
  "success": true,
  "sql": "SELECT strftime('%Y-%m', date) AS Ay, SUM(amount) AS Toplam FROM sales GROUP BY Ay",
...
**Sorgu Geçmişi ve Önbellek işlemleri (`query_history.sqlite`):**
...
| `get_cached_query(query, ttl_hours=24)` | Aynı sorgu TTL süresi içinde geçmişte varsa `{sql, chart_type}` döndürür; yoksa `None` |
...
|-----------|-------|-----------|---------|
| `CACHE_TTL_HOURS` | `database.py` | `24` | Aynı sorgunun önbellekten döndürülebileceği maksimum saat |
...
| Aynı sorguyu 24s içinde tekrar gönderme → önbellekten anında döndü (`from_cache: true`) | ✅ |
| "Önbelleği atla" checkbox'ı → LLM doğrudan çağrıldı, önbellek atlandı | ✅ |
| Önbellekten dönen sonuçta UI'da ⚡ badge göründü | ✅ |
...
- [x] **Sorgu Önbellekleme** — TTL tabanlı `get_cached_query()`; aynı sorgu 24s içinde LLM'e gitmeden döndürülür; `use_cache` ile kullanıcı kontrolü
...
### v3.2.0 — Sorgu Önbellekleme

- **TTL tabanlı önbellek:** `get_cached_query(query, ttl_hours=24)` — aynı sorgu 24 saat içinde tekrar gelirse LLM atlanır, geçmiş SQL yeniden çalıştırılır
- **`use_cache` parametresi:** `POST /api/generate_sql`'e `"use_cache": false` göndererek önbellek devre dışı bırakılabilir
- **"Önbelleği atla" toggle:** Input alanında checkbox; işaretliyse önbellek aktif, kaldırılırsa her sorguda LLM çağrılır
- **⚡ Önbellekten badge:** Önbellekten gelen sonuçlarda sonuç başlığında sarı badge gösterilir; `attempts: 0` ile ayırt edilir
- **Sıfır şema değişikliği:** Mevcut `history` tablosu altyapısı olduğu gibi kullanıldı
