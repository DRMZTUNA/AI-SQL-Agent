...
│ • Chat Hist.  │   │ • Mock DB Setup           │   │ • OllamaEmbeddings      │
│               │   │ • Query History (SQLite)  │   │ • Eski klasör temizliği │
└────────┬──────┘   └───────────────┬──────────┘   └─────────┬──────────────┘
         │                          │                         │
         └──────────────────────────┴─────────────────────────┘
                                    │
...
| 7 | `database.py` | Başarılı sorgu `query_history.sqlite` tablosuna otomatik kaydedilir |
| 8 | Frontend | Sayfalı veri tablosu + Chart.js grafiği render edilir |
...
4. loop.run_in_executor() ile Ollama'ya async gönder (temperature=0, format=json)
5. SQL boşsa → is_chat:True döndür (fallback)
6. SQL doluysa → execute_query() çalıştır
7. Hata varsa → hata + önceki reasoning ile yeniden dene (max_retries=3)
```

**Async Mimarisi:**

`llm.invoke()` senkron bir çağrıdır. FastAPI'nin async event loop'unu bloke etmemek için `asyncio.get_event_loop().run_in_executor(None, llm.invoke, prompt)` ile thread pool'a taşınmıştır. Böylece LLM işlemi sırasında diğer API istekleri yanıtlanmaya devam eder.
...
|-----------|-------|-----------|---------|
| `max_retries` | `agent.py` | `3` | Self-correction döngüsü maksimum deneme sayısı |
